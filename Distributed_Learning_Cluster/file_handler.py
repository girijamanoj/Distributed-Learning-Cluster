...
class file_handler:
    ...
    def append_data(self, results, rep=4):
        try:
            counter = []
            ...
            for node in nodes:
                try:
                    ...
                    continue
                thread = threading.Thread(target=temp_obj.append, daemon=True)
            ...
            if len(counter) == len(nodes):
                return 1
            self.append_data(results, rep=len(nodes) - len(counter))
        except Exception as e:
            print("Error in file_handler_append: ", e)
