...
def get_all_files_for_dataset(file_name):
    "return all versions of file_name"
    normal_file_name, ext, _ = get_file_name_ext_version(file_name)
    files = os.listdir("./")
    temp = []
    for file in files:
        if file.startswith(normal_file_name) and file.endswith(ext):
            temp.append(file)
    if len(temp) == 0:
        return file_name
    temp.sort()
...
def get_ml_cord(member_list):
    for member in member_list:
        if member.startswith("fa22-cs425-1901"):
            return "fa22-cs425-1901"
    return "fa22-cs425-1902"


from message import message


def adjust_dict_cord(payload):
    payload["buffer"] = ""
    while True:
        temp_payload = message(17, payload)
        payload_len = len(temp_payload.get_encoded_message())
        if payload_len == 4096 * 4:
            break
        payload["buffer"] += " "
    return payload


def adjust_dict(payload):
    payload["buffer"] = ""
    while True:
        temp_payload = message(17, payload)
        payload_len = len(temp_payload.get_encoded_message())
